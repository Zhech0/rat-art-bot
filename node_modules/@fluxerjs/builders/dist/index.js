...
function toEmbedMedia(input) {
  ...
  if (!URL.canParse(input.url)) {
    throw new import_util.FluxerError("Invalid embed media URL", { code: import_util.ErrorCodes.InvalidEmbedMediaUrl });
  }
...
var EmbedBuilder = class _EmbedBuilder {
  ...
  setURL(url) {
    if (url != null && url !== "" && !URL.canParse(url)) {
      throw new import_util.FluxerError("Invalid embed URL", { code: import_util.ErrorCodes.InvalidEmbedUrl });
    }
...
var AttachmentBuilder = class {
  ...
  constructor(id, filename, options) {
    if (!filename?.trim()) {
      throw new import_util2.FluxerError("Filename is required", {
        code: import_util2.ErrorCodes.AttachmentFilenameRequired
      });
    }
    this.id = id;
