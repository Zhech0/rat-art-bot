...
function getDefaultWebSocketSync() {
  ...
  throw new import_util.FluxerError(
    ...
    { code: import_util.ErrorCodes.WebSocketLoadFailed }
  );
}
  ...
  handlePayload(payload) {
    switch (payload.op) {
      ...
      case import_types.GatewayOpcodes.Reconnect:
        ...
        break;
      case import_types.GatewayOpcodes.GatewayError: {
        const detail = typeof payload.d === "string" ? payload.d : payload.d !== void 0 ? JSON.stringify(payload.d) : "unknown gateway error";
...
function shouldReconnectOnClose(code) {
  switch (code) {
    ...
    // Sharding required
    case 4012:
    // Invalid API version (after fix, reconnect is safe)
    case 4013:
      return true;
    default:
      return false;
...
var WebSocketManager = class extends import_events2.EventEmitter {
  ...
  async connect() {
    ...
    if (!WS) {
      ...
      if (this._aborted) {
        throw new import_util2.FluxerError("Connection aborted", { code: import_util2.ErrorCodes.GatewayConnectionAborted });
      }
      if (!WS) {
        throw new import_util2.FluxerError("Failed to load WebSocket", { code: import_util2.ErrorCodes.WebSocketLoadFailed });
      }
    }
    let gateway = null;
    while (!this._aborted) {
    ...
    if (!gateway) {
      throw new import_util2.FluxerError("Failed to fetch gateway", { code: import_util2.ErrorCodes.GatewayFetchFailed });
    }
