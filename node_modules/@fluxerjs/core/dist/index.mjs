...
var MessageManager = class {
  ...
  async fetchMany(options) {
    ...
    const qs = params.toString();
    const route = Routes2.channelMessages(this.channelId) + (qs ? `?${qs}` : "");
    const data = await this.client.rest.get(route);
...
var Channel = class _Channel extends Base {
  ...
  async fetchRtcRegions() {
    return this.client.rest.get(Routes5.channelRtcRegions(this.id), { auth: true });
  }
  /**
   ...
   */
  async fetchSlowmode() {
    return this.client.rest.get(Routes5.channelSlowmode(this.id), { auth: true });
  }
  /**
...
var GuildChannel = class extends Channel {
  ...
  async send(options) {
    const payload = await prepareMessagePostPayload(options);
    const data = await this.client.rest.post(Routes5.channelMessages(this.id), payload);
...
// src/client/ChannelManager.ts
var ChannelManager = class extends Collection5 {
  constructor(client) {
    super();
    this.client = client;
  ...
  async send(channelId, payload) {
    ...
    const data = await this.client.rest.post(Routes6.channelMessages(channelId), postPayload);
...
// src/client/GuildMemberManager.ts
var GuildMemberManager = class extends Collection7 {
  constructor(guild) {
    super();
    this.guild = guild;
  }
  ...
  async search(options) {
    return this.guild.client.rest.post(Routes9.guildMembersSearch(this.guild.id), {
      body: options,
      auth: true
    });
  }
...
var Guild = class extends Base {
  ...
  splashHeight;
  members;
  channels = new Collection8();
  roles = new Collection8();
  emojis = new Collection8();
  stickers = new Collection8();
...
// src/client/GuildManager.ts
var GuildManager = class extends Collection9 {
  constructor(client) {
    super();
    this.client = client;
...
import { Collection as Collection10 } from "@fluxerjs/collection";
import { Routes as Routes18 } from "@fluxerjs/types";
var UsersManager = class extends Collection10 {
  constructor(client) {
    super();
    this.client = client;
...
function isChatInputCommandInteraction(interaction) {
...
// src/index.ts
import { EmbedBuilder as EmbedBuilder4, MessagePayload, AttachmentBuilder } from "@fluxerjs/builders";
import {
  Routes as Routes21,
  GatewayOpcodes,
