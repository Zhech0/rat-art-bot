...
// src/subpath-client.ts
var subpath_client_exports = {};
__export(subpath_client_exports, {
  Client: () => Client,
  Events: () => Events
});
module.exports = __toCommonJS(subpath_client_exports);
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if ("url" in f && f.url) {
      if (!URL.canParse(f.url)) {
        throw new import_util.FluxerError(`Invalid file URL at index ${i}: ${f.url}`, {
      ...
      try {
        ...
        throw new import_util.FluxerError(`Failed to fetch file from ${f.url}`, {
          code: import_util2.ErrorCodes.FileFetchFailed,
          cause: err instanceof Error ? err : void 0
        });
      }
      if (!res.ok) {
        throw new import_util.FluxerError(
      ...
      throw new import_util.FluxerError(`File at index ${i} must have either "data" or "url"`, {
...
var Message = class _Message extends Base {
  ...
  async _send(payload) {
    const data = await this.client.rest.post(
      import_types2.Routes.channelMessages(this.channelId),
      payload
    );
  ...
  async deleteAttachment(attachmentId) {
    await this.client.rest.delete(
      import_types2.Routes.channelMessageAttachment(this.channelId, this.id, attachmentId),
      { auth: true }
    );
  ...
  async fetchReactionUsers(emoji, options) {
    ...
    const route = import_types2.Routes.channelMessageReaction(this.channelId, this.id, emojiStr) + (qs ? `?${qs}` : "");
...
var MessageManager = class {
  ...
  async fetch(messageIdOrOptions) {
  ...
  async fetchOne(messageId) {
    try {
      const data = await this.client.rest.get(
        import_types3.Routes.channelMessage(this.channelId, messageId)
      );
      this.client._addMessageToCache(this.channelId, data);
      ...
      if (err instanceof import_rest.FluxerAPIError && err.statusCode === 404) {
        throw new import_util.FluxerError(`Message ${messageId} not found in channel ${this.channelId}`, {
          code: import_util2.ErrorCodes.MessageNotFound,
          cause: err
        });
      }
      throw err instanceof import_util.FluxerError ? err : new import_util.FluxerError(String(err), { cause: err });
    }
  }
  async fetchMany(options) {
    ...
    const qs = params.toString();
    const route = import_types3.Routes.channelMessages(this.channelId) + (qs ? `?${qs}` : "");
    const data = await this.client.rest.get(route);
...
var Webhook = class _Webhook extends Base {
  ...
  async delete() {
    await this.client.rest.delete(import_types4.Routes.webhook(this.id), { auth: true });
  }
  /**
  ...
  async edit(options) {
    ...
    if (this.token) {
      const data2 = await this.client.rest.patch(import_types4.Routes.webhookExecute(this.id, this.token), {
        body,
        auth: false
      });
      const w2 = data2;
    ...
    const data = await this.client.rest.patch(import_types4.Routes.webhook(this.id), {
      body,
      auth: true
    });
    const w = data;
  ...
  async send(options, wait) {
    ...
    const route = import_types4.Routes.webhookExecute(this.id, this.token) + (wait ? "?wait=true" : "");
  ...
  async editMessage(messageId, options) {
    ...
    if (options.embeds !== void 0) {
      body.embeds = options.embeds.map((e) => e instanceof import_builders3.EmbedBuilder ? e.toJSON() : e);
    }
    const data = await this.client.rest.patch(
      import_types4.Routes.webhookMessage(this.id, this.token, messageId),
      { body, auth: false }
    );
    return new Message(this.client, data);
  }
  ...
  async deleteMessage(messageId) {
    ...
    await this.client.rest.delete(import_types4.Routes.webhookMessage(this.id, this.token, messageId), {
      auth: false
    });
  }
  /**
  ...
  static async fetch(client, webhookId) {
    const data = await client.rest.get(import_types4.Routes.webhook(webhookId));
...
var Invite = class _Invite extends Base {
  ...
  static normalizeCode(codeOrUrl) {
    ...
    if (!input) {
      throw new import_util.FluxerError("Invite code cannot be empty", { code: import_util2.ErrorCodes.InvalidInvite });
    }
    const parseInviteCodeFromUrl = (value) => {
    ...
    const fromHostLike = !hasScheme && URL.canParse(`https://${input}`) ? parseInviteCodeFromUrl(`https://${input}`) : null;
    const directCode = decodeURIComponent(input).trim();
    const code = fromAbsolute ?? fromHostLike ?? directCode;
    if (!code || /[\s/?#]/.test(code)) {
      throw new import_util.FluxerError("Invalid invite code or URL", { code: import_util2.ErrorCodes.InvalidInvite });
    }
    return code;
  }
  ...
  static async fetch(client, codeOrUrl) {
    ...
    const data = await client.rest.get(import_types5.Routes.invite(code));
   ...
   */
  async delete() {
    await this.client.rest.delete(import_types5.Routes.invite(this.code), { auth: true });
  }
};
...
var Channel = class _Channel extends Base {
  ...
  isDM() {
    return this.type === import_types6.ChannelType.DM || this.type === import_types6.ChannelType.GroupDM;
  }
  ...
  static from(client, data) {
    ...
    if (type === import_types6.ChannelType.GuildText) return new TextChannel(client, data);
    if (type === import_types6.ChannelType.GuildCategory) return new CategoryChannel(client, data);
    if (type === import_types6.ChannelType.GuildVoice) return new VoiceChannel(client, data);
    if (type === import_types6.ChannelType.GuildLink || type === import_types6.ChannelType.GuildLinkExtended)
  ...
  static fromOrCreate(client, data) {
    const type = data.type ?? 0;
    if (type === import_types6.ChannelType.DM || type === import_types6.ChannelType.GroupDM)
  ...
  async bulkDeleteMessages(messageIds) {
    await this.client.rest.post(import_types6.Routes.channelBulkDelete(this.id), {
   ...
   */
  async sendTyping() {
    await this.client.rest.post(import_types6.Routes.channelTyping(this.id), { auth: true });
  }
  /**
  ...
  async fetchRtcRegions() {
    return this.client.rest.get(import_types6.Routes.channelRtcRegions(this.id), { auth: true });
  }
  /**
   ...
   */
  async fetchSlowmode() {
    return this.client.rest.get(import_types6.Routes.channelSlowmode(this.id), { auth: true });
  }
  /**
...
var GuildChannel = class extends Channel {
  ...
  async createWebhook(options) {
    const data = await this.client.rest.post(import_types6.Routes.channelWebhooks(this.id), {
      body: options,
      auth: true
    });
   ...
   */
  async fetchWebhooks() {
    const data = await this.client.rest.get(import_types6.Routes.channelWebhooks(this.id));
  ...
  async createInvite(options) {
    ...
    const data = await this.client.rest.post(import_types6.Routes.channelInvites(this.id), {
   ...
   */
  async fetchInvites() {
    const data = await this.client.rest.get(import_types6.Routes.channelInvites(this.id));
    const list = Array.isArray(data) ? data : Object.values(data ?? {});
  ...
  async editPermission(overwriteId, options) {
    await this.client.rest.put(import_types6.Routes.channelPermission(this.id, overwriteId), {
      body: options,
      auth: true
    });
  ...
  canSendMessage() {
    ...
    return perms.has(import_util3.PermissionFlags.ViewChannel) && perms.has(import_util3.PermissionFlags.SendMessages);
  }
  /**
  ...
  async send(options) {
    const payload = await prepareMessagePostPayload(options);
    const data = await this.client.rest.post(import_types6.Routes.channelMessages(this.id), payload);
  ...
  async deletePermission(overwriteId) {
    await this.client.rest.delete(import_types6.Routes.channelPermission(this.id, overwriteId), { auth: true });
    const idx = this.permissionOverwrites.findIndex((o) => o.id === overwriteId);
   ...
   */
  async edit(options) {
    const data = await this.client.rest.patch(import_types6.Routes.channel(this.id), {
      body: options,
      auth: true
    });
  ...
  async delete(options) {
    const url = import_types6.Routes.channel(this.id) + (options?.silent ? "?silent=true" : "");
...
var TextChannel = class extends GuildChannel {
  ...
  async fetchPinnedMessages() {
    const data = await this.client.rest.get(import_types6.Routes.channelPins(this.id));
   ...
   */
  async fetchMessage(messageId) {
    (0, import_util4.emitDeprecationWarning)(
...
var DMChannel = class extends Channel {
  ...
  async addRecipient(userId) {
    await this.client.rest.put(import_types6.Routes.channelRecipient(this.id, userId), { auth: true });
  ...
  async removeRecipient(userId, options) {
    const url = import_types6.Routes.channelRecipient(this.id, userId) + (options?.silent ? "?silent=true" : "");
    await this.client.rest.delete(url, { auth: true });
...
// src/client/ChannelManager.ts
var ChannelManager = class extends import_collection5.Collection {
  constructor(client) {
    super();
    this.client = client;
  ...
  async fetch(channelId) {
    ...
    try {
      const data = await this.client.rest.get(import_types7.Routes.channel(channelId));
      const channel = Channel.fromOrCreate(this.client, data);
      if (!channel) {
        throw new import_util.FluxerError("Channel data invalid or unsupported type", {
      ...
      if (err instanceof import_rest2.FluxerAPIError && err.statusCode === 404) {
        throw new import_util.FluxerError(`Channel ${channelId} not found`, {
          code: import_util2.ErrorCodes.ChannelNotFound,
          cause: err
        });
      }
  ...
  async fetchMessage(channelId, messageId) {
    (0, import_util5.emitDeprecationWarning)(
    ...
    try {
      const data = await this.client.rest.get(
        import_types7.Routes.channelMessage(channelId, messageId)
      );
      return new Message(this.client, data);
    } catch (err) {
      if (err instanceof import_rest2.RateLimitError) throw err;
      if (err instanceof import_rest2.FluxerAPIError && err.statusCode === 404) {
        throw new import_util.FluxerError(`Message ${messageId} not found in channel ${channelId}`, {
          code: import_util2.ErrorCodes.MessageNotFound,
          cause: err
        });
      }
  ...
  async send(channelId, payload) {
    ...
    const data = await this.client.rest.post(import_types7.Routes.channelMessages(channelId), postPayload);
...
function computePermissions(basePermissions, overwrites, memberRoles, memberId, isOwner) {
  if (isOwner) return import_util6.ALL_PERMISSIONS_BIGINT;
  let perms = basePermissions;
  for (const overwrite of overwrites ?? []) {
    const applies = overwrite.type === import_types8.OverwriteType.Role && memberRoles.includes(overwrite.id) || overwrite.type === import_types8.OverwriteType.Member && overwrite.id === memberId;
    if (!applies) continue;
    const allow = BigInt(overwrite.allow || "0");
    const deny = BigInt(overwrite.deny || "0");
    perms = perms & ~deny | allow;
  }
  return (perms & import_util6.PermissionFlags.Administrator) !== 0n ? import_util6.ALL_PERMISSIONS_BIGINT : perms;
}
...
var GuildMemberRoleManager = class {
   ...
   */
  get cache() {
    const coll = new import_collection6.Collection();
  ...
  async add(roleOrId) {
    ...
    await this.member.client.rest.put(
      import_types9.Routes.guildMemberRole(this.member.guild.id, this.member.id, roleId)
    );
  ...
  async set(roleIds) {
    const data = await this.member.client.rest.patch(
      import_types9.Routes.guildMember(this.member.guild.id, this.member.id),
...
var GuildMember = class extends Base {
  ...
  async edit(options) {
    ...
    const route = isMe ? `/guilds/${this.guild.id}/members/@me` : import_types10.Routes.guildMember(this.guild.id, this.id);
  ...
  get permissions() {
    ...
    return new import_util7.BitField(perms);
  }
  /**
...
// src/client/GuildMemberManager.ts
var GuildMemberManager = class extends import_collection7.Collection {
  constructor(guild) {
    super();
    this.guild = guild;
  }
  ...
  async fetchMe() {
    const userId = this.guild.client.user?.id;
    if (!userId) {
      throw new import_util.FluxerError("Cannot fetch me: client.user is null (client not ready)", {
  ...
  async fetch(options) {
    ...
    if (options?.after) params.set("after", options.after);
    const qs = params.toString();
    const url = import_types11.Routes.guildMembers(this.guild.id) + (qs ? `?${qs}` : "");
  ...
  async search(options) {
    return this.guild.client.rest.post(import_types11.Routes.guildMembersSearch(this.guild.id), {
      body: options,
      auth: true
    });
  }
...
var Role = class extends Base {
  ...
  get permissions() {
    ...
    return new import_util8.PermissionsBitField(
      (bits & import_util8.PermissionFlags.Administrator) !== 0n ? import_util8.ALL_PERMISSIONS_BIGINT : bits
    );
  }
  ...
  async edit(options) {
    ...
    if (options.permissions !== void 0) {
      body.permissions = typeof options.permissions === "string" ? options.permissions : (0, import_util8.resolvePermissionsToBitfield)(options.permissions);
    }
    ...
    const data = await this.client.rest.patch(import_types12.Routes.guildRole(this.guildId, this.id), {
      body: Object.keys(body).length ? body : void 0,
      auth: true
    });
    this._patch(data);
   ...
   */
  async delete() {
    await this.client.rest.delete(import_types12.Routes.guildRole(this.guildId, this.id), { auth: true });
    const guild = this.client.guilds.get(this.guildId);
...
var GuildBan = class extends Base {
   ...
   */
  async unban() {
    await this.client.rest.delete(import_types13.Routes.guildBan(this.guildId, this.user.id), {
      auth: true
    });
  }
};
...
var Guild = class extends Base {
  ...
  splashHeight;
  members;
  channels = new import_collection8.Collection();
  roles = new import_collection8.Collection();
  emojis = new import_collection8.Collection();
  stickers = new import_collection8.Collection();
  ...
  async addRoleToMember(userId, roleId) {
    await this.client.rest.put(import_types16.Routes.guildMemberRole(this.id, userId, roleId));
  }
  /**
  ...
  async removeRoleFromMember(userId, roleId) {
    await this.client.rest.delete(import_types16.Routes.guildMemberRole(this.id, userId, roleId));
  }
  /**
  ...
  async createRole(options) {
    ...
    if (options.name !== void 0) body.name = options.name;
    if (options.permissions !== void 0) {
      body.permissions = typeof options.permissions === "string" ? options.permissions : (0, import_util9.resolvePermissionsToBitfield)(options.permissions);
    }
    if (options.color !== void 0) body.color = options.color;
    if (options.hoist !== void 0) body.hoist = options.hoist;
    if (options.mentionable !== void 0) body.mentionable = options.mentionable;
    ...
    if (options.position !== void 0) body.position = options.position;
    if (options.hoist_position !== void 0) body.hoist_position = options.hoist_position;
    const data = await this.client.rest.post(import_types16.Routes.guildRoles(this.id), {
      body: Object.keys(body).length ? body : void 0,
      auth: true
    });
  ...
  async fetchRoles() {
    const data = await this.client.rest.get(
      import_types16.Routes.guildRoles(this.id)
    );
    const list = Array.isArray(data) ? data : Object.values(data ?? {});
    const roles = [];
    for (const r of list) {
  ...
  async fetchRole(roleId) {
    try {
      const data = await this.client.rest.get(import_types16.Routes.guildRole(this.id, roleId));
      const role = new Role(this.client, data, this.id);
      this.roles.set(role.id, role);
      return role;
    } catch (err) {
      ...
      if (statusCode === 404) {
        throw new import_util.FluxerError(`Role ${roleId} not found in guild`, {
          code: import_util2.ErrorCodes.RoleNotFound,
          cause: err
        });
      }
      throw err instanceof import_util.FluxerError ? err : new import_util.FluxerError("Failed to fetch guild role", { cause: err });
    }
  }
  /**
  ...
  async resolveRoleId(arg) {
    const parsed = (0, import_util9.parseRoleMention)(arg);
    ...
    const roles = await this.client.rest.get(import_types16.Routes.guildRoles(this.id));
  ...
  async ban(userId, options) {
    ...
    await this.client.rest.put(import_types16.Routes.guildBan(this.id, userId), {
      body: Object.keys(body).length ? body : void 0,
      auth: true
    });
  }
  ...
  async fetchBans() {
    const data = await this.client.rest.get(
      import_types16.Routes.guildBans(this.id)
    );
   ...
   */
  async unban(userId) {
    await this.client.rest.delete(import_types16.Routes.guildBan(this.id, userId), { auth: true });
  }
  /**
   ...
   */
  async kick(userId) {
    await this.client.rest.delete(import_types16.Routes.guildMember(this.id, userId), { auth: true });
  }
  /**
  ...
  async fetchMember(userId) {
    try {
      const data = await this.client.rest.get(
        import_types16.Routes.guildMember(this.id, userId)
      );
      ...
      const statusCode = err instanceof import_rest3.FluxerAPIError ? err.statusCode : err?.statusCode;
      if (statusCode === 404) {
        throw new import_util.FluxerError(`Member ${userId} not found in guild`, {
          code: import_util2.ErrorCodes.MemberNotFound,
          cause: err
        });
      }
      throw err instanceof import_util.FluxerError ? err : new import_util.FluxerError("Failed to fetch guild member", { cause: err });
    }
  }
  /**
  ...
  async fetchAuditLogs(options) {
    ...
    const qs = params.toString();
    const url = import_types16.Routes.guildAuditLogs(this.id) + (qs ? `?${qs}` : "");
    return this.client.rest.get(url);
  }
  /** Fetch all webhooks in this guild. Returned webhooks do not include the token (cannot send). */
  async fetchWebhooks() {
    const data = await this.client.rest.get(import_types16.Routes.guildWebhooks(this.id));
    const list = Array.isArray(data) ? data : Object.values(data ?? {});
    return list.map((w) => new Webhook(this.client, w));
  }
  /**
  ...
  async fetchInvites() {
    const data = await this.client.rest.get(
      import_types16.Routes.guildInvites(this.id)
    );
    const list = Array.isArray(data) ? data : Object.values(data ?? {});
  ...
  async createChannel(data) {
    const created = await this.client.rest.post(import_types16.Routes.guildChannels(this.id), {
      body: data,
      auth: true
    });
   ...
   */
  async fetchChannels() {
    const data = await this.client.rest.get(import_types16.Routes.guildChannels(this.id));
    const list = Array.isArray(data) ? data : Object.values(data ?? {});
    const channels = [];
   ...
   */
  async edit(options) {
    const data = await this.client.rest.patch(import_types16.Routes.guild(this.id), {
      body: options,
      auth: true
    });
    this.name = data.name;
   ...
   */
  async delete() {
    await this.client.rest.post(import_types16.Routes.guildDelete(this.id), { auth: true });
  ...
  async fetchVanityURL() {
    return this.client.rest.get(import_types16.Routes.guildVanityUrl(this.id), { auth: true });
  }
  /**
  ...
  async transferOwnership(newOwnerId, password) {
    await this.client.rest.post(import_types16.Routes.guildTransferOwnership(this.id), {
  ...
  setTextChannelFlexibleNames(enabled) {
    return this.client.rest.patch(import_types16.Routes.guildTextChannelFlexibleNames(this.id), {
      body: { enabled },
      auth: true
    }).then(() => this);
  }
  ...
  setDetachedBanner(enabled) {
    return this.client.rest.patch(import_types16.Routes.guildDetachedBanner(this.id), {
      body: { enabled },
      auth: true
    }).then(() => this);
  }
  ...
  setDisallowUnclaimedAccounts(enabled) {
    return this.client.rest.patch(import_types16.Routes.guildDisallowUnclaimedAccounts(this.id), {
      body: { enabled },
      auth: true
    }).then(() => this);
  }
  ...
  async setRoleHoistPositions(updates) {
    const data = await this.client.rest.patch(
      import_types16.Routes.guildRolesHoistPositions(this.id),
      { body: updates, auth: true }
    );
    const list = Array.isArray(data) ? data : Object.values(data ?? {});
    for (const r of list) {
  ...
  async setChannelPositions(updates) {
    await this.client.rest.patch(import_types16.Routes.guildChannels(this.id), {
      body: updates,
      auth: true
    });
  }
  ...
  async fetchEmojis() {
    const data = await this.client.rest.get(
      import_types16.Routes.guildEmojis(this.id)
    );
    const list = Array.isArray(data) ? data : Object.values(data ?? {});
    const emojis = [];
    for (const e of list) {
  ...
  async fetchEmoji(emojiId) {
    try {
      const data = await this.client.rest.get(import_types16.Routes.guildEmoji(this.id, emojiId));
      ...
      const statusCode = err instanceof import_rest3.FluxerAPIError ? err.statusCode : err?.statusCode;
      if (statusCode === 404) {
        throw new import_util.FluxerError(`Emoji ${emojiId} not found in guild`, {
          code: import_util2.ErrorCodes.EmojiNotFound,
          cause: err
        });
      }
      throw err instanceof import_util.FluxerError ? err : new import_util.FluxerError("Failed to fetch guild emoji", { cause: err });
    }
  }
  /**
  ...
  async createEmojisBulk(emojis) {
    const data = await this.client.rest.post(
      import_types16.Routes.guildEmojisBulk(this.id),
      {
        body: { emojis },
        auth: true
      }
  ...
  async createStickersBulk(stickers) {
    const data = await this.client.rest.post(
      import_types16.Routes.guildStickersBulk(this.id),
      {
        body: { stickers },
        auth: true
      }
  ...
  async fetchStickers() {
    const data = await this.client.rest.get(
      import_types16.Routes.guildStickers(this.id)
    );
    const list = Array.isArray(data) ? data : Object.values(data ?? {});
    const stickers = [];
    for (const s of list) {
  ...
  async fetchSticker(stickerId) {
    const data = await this.client.rest.get(import_types16.Routes.guildSticker(this.id, stickerId));
...
// src/client/GuildManager.ts
var GuildManager = class extends import_collection9.Collection {
  constructor(client) {
    super();
    this.client = client;
  ...
  async create(options) {
    const data = await this.client.rest.post(import_types17.Routes.guilds(), {
      body: options,
      auth: true
    });
  ...
  async fetch(guildId) {
    ...
    if (cached) return cached;
    try {
      const data = await this.client.rest.get(import_types17.Routes.guild(guildId));
      const guild = new Guild(this.client, data);
      this.set(guild.id, guild);
      return guild;
    } catch {
...
var User = class extends Base {
   ...
   */
  async createDM() {
    const data = await this.client.rest.post(import_types18.Routes.userMeChannels(), {
...
var ClientUser = class extends User {
  ...
  async fetchGuilds() {
    const data = await this.client.rest.get(
      import_types19.Routes.currentUserGuilds()
    );
...
var UsersManager = class extends import_collection10.Collection {
  constructor(client) {
    super();
    this.client = client;
   ...
   */
  async fetch(userId) {
    const data = await this.client.rest.get(import_types20.Routes.user(userId));
  ...
  async fetchWithProfile(userId, options) {
    ...
    const [userData, globalProfileData, serverProfileData, memberData] = await Promise.all([
      this.client.rest.get(import_types20.Routes.user(userId)),
      this.client.rest.get(import_types20.Routes.userProfile(userId)).catch(() => null),
      guildId ? this.client.rest.get(import_types20.Routes.userProfile(userId, guildId)).catch(() => null) : Promise.resolve(null),
      guildId ? this.client.rest.get(import_types20.Routes.guildMember(guildId, userId)).catch(() => null) : Promise.resolve(null)
    ]);
...
// src/structures/interactions/BaseInteraction.ts
...
// src/structures/interactions/ChatInputCommandInteraction.ts
...
function findOptionDeep(options, name) {
  if (!options?.length) return void 0;
  for (const o of options) {
    if (o.name === name && o.value !== void 0) return o;
    const nested = findOptionDeep(o.options, name);
    if (nested) return nested;
  }
  return void 0;
}
var ChatInputCommandInteraction = class extends BaseInteraction {
  constructor(client, raw) {
    super(client, raw);
    if (raw.data?.name == null) {
      throw new import_util.FluxerError(
        ...
        {
          code: import_util2.ErrorCodes.InteractionInvalidPayload
        }
      );
    }
  }
  ...
  getOption(name) {
    const opt = findOptionDeep(this.raw.data?.options, name);
    if (!opt) {
      throw new import_util.FluxerError(`Interaction option "${name}" was not provided`, {
        code: import_util2.ErrorCodes.InteractionOptionNotFound
      });
    }
    return opt;
  }
  getString(name) {
    ...
    if (o.type !== CommandOptionType.String) {
      throw new import_util.FluxerError(`Option "${name}" is not a string`, {
        code: import_util2.ErrorCodes.InteractionOptionTypeMismatch
      });
    }
  ...
  getInteger(name) {
    ...
    if (o.type !== CommandOptionType.Integer) {
      throw new import_util.FluxerError(`Option "${name}" is not an integer`, {
        code: import_util2.ErrorCodes.InteractionOptionTypeMismatch
      });
    }
    const v = o.value;
    if (typeof v !== "number" || !Number.isInteger(v)) {
      throw new import_util.FluxerError(`Option "${name}" has invalid integer value`, {
        code: import_util2.ErrorCodes.InteractionOptionTypeMismatch
      });
    }
    return v;
  ...
  getNumber(name) {
    ...
    if (o.type !== CommandOptionType.Number && o.type !== CommandOptionType.Integer) {
      throw new import_util.FluxerError(`Option "${name}" is not a number`, {
        code: import_util2.ErrorCodes.InteractionOptionTypeMismatch
      });
    }
    const v = o.value;
    if (typeof v !== "number") {
      throw new import_util.FluxerError(`Option "${name}" has invalid number value`, {
        code: import_util2.ErrorCodes.InteractionOptionTypeMismatch
      });
    }
    return v;
  ...
  getBoolean(name) {
    ...
    if (o.type !== CommandOptionType.Boolean) {
      throw new import_util.FluxerError(`Option "${name}" is not a boolean`, {
        code: import_util2.ErrorCodes.InteractionOptionTypeMismatch
      });
    }
    if (typeof o.value !== "boolean") {
      throw new import_util.FluxerError(`Option "${name}" has invalid boolean value`, {
        code: import_util2.ErrorCodes.InteractionOptionTypeMismatch
      });
    }
    return o.value;
  ...
  getSubcommand(fallback) {
    ...
    throw new import_util.FluxerError("No subcommand in this interaction", {
      code: import_util2.ErrorCodes.InteractionOptionNotFound
    });
  }
};

// src/structures/interactions/createInteraction.ts
...
var MessageReaction = class extends Base {
  ...
  async fetchMessage() {
    try {
      const data = await this.client.rest.get(
        import_types21.Routes.channelMessage(this.channelId, this.messageId)
      );
      return new Message(this.client, data);
    } catch (err) {
      if (err instanceof import_rest4.RateLimitError) throw err;
      if (err instanceof import_rest4.FluxerAPIError && err.statusCode === 404) {
        throw new import_util.FluxerError(`Message ${this.messageId} not found in channel ${this.channelId}`, {
          code: import_util2.ErrorCodes.MessageNotFound,
          cause: err
        });
      }
...
var Client = class extends import_events3.EventEmitter {
  ...
  async resolveEmoji(emoji, guildId) {
    if (typeof emoji === "object" && emoji.id) {
      ...
      return (0, import_util10.formatEmoji)({ name: emoji.name, id: emoji.id, animated: emoji.animated });
    }
    const parsed = (0, import_util10.parseEmoji)(
      typeof emoji === "string" ? emoji : emoji.id ? `:${emoji.name}:` : emoji.name
    );
    if (!parsed) {
      throw new import_util.FluxerError("Invalid emoji", { code: import_util2.ErrorCodes.InvalidEmoji });
    }
    if (parsed.id) {
      if (guildId) {
        await this.assertEmojiInGuild(parsed.id, guildId);
      }
      return (0, import_util10.formatEmoji)(parsed);
    }
    if (!/^\w+$/.test(parsed.name)) return parsed.name;
    const unicodeFromShortcode = (0, import_util10.getUnicodeFromShortcode)(parsed.name);
    if (unicodeFromShortcode) return unicodeFromShortcode;
    if (guildId) {
      const emojis = await this.rest.get(import_types22.Routes.guildEmojis(guildId));
      ...
      if (found) return (0, import_util10.formatEmoji)({ ...parsed, id: found.id, animated: found.animated });
      throw new import_util.FluxerError(
  ...
  async assertEmojiInGuild(emojiId, guildId) {
    ...
    if (!found) {
      throw new import_util.FluxerError("Custom emoji is from another server. Use an emoji from this server.", {
        code: import_util2.ErrorCodes.EmojiNotInGuild
      });
    }
  }
  /**
   ...
   */
  async fetchInstance() {
    return this.rest.get(import_types22.Routes.instance(), { auth: false });
  }
  /**
   * Fetch instance discovery document (API URL, gateway URL, features). GET /.well-known/fluxer.
   * Does not require authentication.
   */
  async fetchInstanceDiscovery() {
    return this.rest.get(import_types22.Routes.instanceDiscovery(), { auth: false });
  }
  /**
  ...
  async fetchGatewayInfo() {
    return this.rest.get(import_types22.Routes.gatewayBot());
  }
  /**
   ...
   */
  async fetchMessage(channelId, messageId) {
    (0, import_util10.emitDeprecationWarning)(
      "Client.fetchMessage()",
  ...
  get ws() {
    if (!this._ws) {
      throw new import_util.FluxerError("Client is not logged in. Call login() first.", {
   ...
   */
  _shouldDeferGatewayDispatchUntilReady(payload) {
  ...
  async _flushDeferredGatewayDispatches() {
  ...
  async login(token) {
    if (this._ws) {
      throw new import_util.FluxerError("Client is already logged in. Call destroy() first.", {
  ...
  _finalizeReady() {
    ...
    void this._flushDeferredGatewayDispatches().catch(
      (err) => this.emit(Events.Error, err instanceof Error ? err : new Error(String(err)))
    );
  }
  /**
